...
__interrupt void UART1_RX_ISR(void)
{
  ...
  data=U1RXBUF;  
  if(rev_flag)
    uartstruct.rev_str[uartstruct.cnt++]=data;
  
  if(uartstruct.cnt>=80)
    rev_flag =0;
#endif
} 
...
#if 0
      rev_flag=1;       //start receive data from uart1
      while(rev_flag);  //receive data finish
      
      for(i=0;i<80;i++)
      {
        if( (uartstruct.rev_str[i]==0x55) && (uartstruct.rev_str[i+1]==0xff) && (uartstruct.rev_str[i+2]==0x55))       
      }
      for(;i<80;i++)
      {
        send_str[i]=uartstruct.rev_str[i];
      }
